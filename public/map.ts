...
const CENTER: [number, number] = [14.6816, 120.9741];
...
function addCandidate(site: CandidateSite) {
...
function addSentinel(sentinel: Sentinel) {
...
function addPerimeter(label: string, radius: number, perimeterColor: string, dashArray: string) {
...
function renderStepRail() {
  ...
  document.querySelectorAll('[data-step]').forEach((node) => {
    const activate = () => {
      state.step = Number((node as HTMLElement).dataset.step);
      render();
    };
