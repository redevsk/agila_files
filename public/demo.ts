...
const CENTER: [number, number] = [14.6834, 120.9748];
...
function colorFor(value: string) {
  return ({
    ...
    report: '#be123c',
    sentinel: '#0f766e',
  } as Record<string, string>)[value] || '#2563eb';
}
...
function addZone(zone: Zone, options: { color?: string; fillOpacity?: number; weight?: number } = {}) {
...
function addCandidateSite(site: CandidateSite) {
...
function addSentinel(sentinel: Sentinel, large = false) {
  const icon = L.divIcon({
    className: '',
...
function addPerimeter(label: string, radius: number, color: string, dashArray: string) {
...
function renderStepRail() {
  ...
  document.querySelectorAll('[data-step]').forEach((button) => {
    button.addEventListener('click', () => {
      state.step = Number((button as HTMLElement).dataset.step);
...
function pulse(center: [number, number], variant = '') {
...
function addInspectorMarker(center: [number, number]) {
  const icon = L.divIcon({
    className: '',
...
function resetDemo() {
  ...
  state.step = 0;
  state.pulseLayer?.clearLayers();
  (qs('nextButton') as HTMLButtonElement).disabled = false;
